...
> **Namespaces:** configures OIDC auth in the root and `admin` namespaces, with team secrets in `admin/tn001`.
...
- Adds existing users to the groups listed for them in `existing_users`
...
graph TB
    subgraph Azure["Azure Entra ID"]
        ...
        GROUPS["Azure AD Groups<br/>- vault-admin<br/>- vault-user<br/>- vault-tn001-team1-reader<br/>- vault-tn001-team2-reader"]
        USERS["Azure AD Users<br/>- Created Users<br/>- Existing Users"]
    end

    subgraph Vault["HashiCorp Vault"]
        OIDC["OIDC Auth Method<br/>/auth/oidc"]
        IE["Identity Entities<br/>(per user)"]
        IG["Identity Groups<br/>- entra-vault-admin-external<br/>- entra-vault-tn001-team1-reader-external<br/>- entra-vault-tn001-team1-reader-internal"]
        POL["Policies<br/>- entra-admin<br/>- entra-tn001-team1-reader"]
        SEC["Secrets<br/>admin/tn001: team1/*"]
    end
...
sequenceDiagram
    ...
    User->>Vault: vault login -method=oidc role=default
...
- Terraform CLI
- Azure service principal credentials exported as `ARM_*` environment variables (see [Configure Azure Entra ID](#configure-azure-entra-id))

## Quick Start

Complete the Azure setup in [Configure Azure Entra ID](#configure-azure-entra-id) first, then copy `terraform.tfvars.example` to `terraform.tfvars` and customize it.
...
4. Grant Azure AD permissions to the service principal:
...
5. Set environment variables using the output from step 3:
...
6. Source the variables:

   ```bash
   source .env
   ```

### Grant Admin Consent for the Vault Application

After `terraform apply`, grant admin consent for the Vault application (if not done earlier):
...
vault login -method=oidc -namespace=admin role=default
```

### Namespace-Aware Authentication
...
### Identity Groups Across Namespaces
...
**Admin Namespace (`auth/oidc` in admin):**
...
### Authenticate in the Admin Namespace
...
```bash
vault write -namespace=admin auth/oidc/oidc/auth role=default \
...
**Authentication Demo (`demo-auth.sh`)** demonstrates the difference between root and admin namespace authentication:
...
**Policy Inspector (`check-policies.sh`)** analyzes your current token and shows detailed policy information:
...
| `existing_users` | Map of existing Azure AD users (keyed by UPN prefix; the tenant's default domain is appended) with the groups to add them to |
| `vault_oidc_mount_path` | Path of the OIDC auth method in the root and admin namespaces (default: `oidc`) |

### File Structure

- `main.tf`: Azure AD application registration, client secret and service principal
- `users.tf`: Azure AD groups, users and group memberships
- `vault.tf`: Vault namespaces and OIDC auth methods/roles (root and admin)
- `identity.tf`: Vault identity groups and group aliases for Azure groups
...
**Solution:** Authenticate in the correct namespace where your group aliases are defined:

```bash
vault login -namespace=admin -method=oidc role=default
```

### Cannot Access Nested Namespace Secrets
...
**Solution:** Ensure you authenticated in admin namespace and are a member of the corresponding Azure AD group.
