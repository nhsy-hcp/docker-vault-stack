...
> Network alias update: the Docker stack now exposes localhost-style aliases. This lab uses `authentik.localhost:9000` for host and browser access, and `authentik.localhost:9000` as the Vault OIDC discovery default.
...
- Authentik host access uses `authentik.localhost:9000`
