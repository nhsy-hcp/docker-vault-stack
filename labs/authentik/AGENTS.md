...
1. **Terraform Provider** (runs on host):
   - Uses `authentik_url = http://authentik.localhost:9000`
   - Connects via port forwarding to host

2. **Vault OIDC** (runs in container):
   - Uses `vault_authentik_url = http://authentik.localhost:9000`
   - Connects via the Docker network alias on `docker-vault-stack`
...
- `.env` sets `AUTHENTIK_URL=http://authentik.localhost:9000` for Terraform provider
- Vault resolves `authentik.localhost` via the Docker network alias

**Why This Matters**:
- Using `localhost` in Vault OIDC config fails (Vault can't reach host)
- Using an internal-only Docker service name in the Terraform provider fails from the host
...
1. **Authentik UI**: Accessible at http://authentik.localhost:9000
...
curl -f http://authentik.localhost:9000/-/health/live/

# 2. Verify admin user can login
# Login via UI: http://authentik.localhost:9000
